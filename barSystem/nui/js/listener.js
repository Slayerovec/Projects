    ...
    for (k in restaurantData.alcohol) {
        if (k != 'Alcohol'){
            ...
            for (drink in restaurantData.alcohol[k]) {
                prefix = restaurantData.alcohol[k][drink]
                if (changed != false) {
                    if (prefix.unique === false) {
                        if (prefix.id == changed){
...
function createMenuItems() {
    for (k in restaurantData.alcohol) {
       name = k.toLowerCase();
       $('.flipbook').append('<div class="hard"  style="background-image:url(img/background.png)" id="type"><img src="img/'+name+'.png"></div>');
       for (drink in restaurantData.alcohol[k]) {
            prefix = restaurantData.alcohol[k][drink]
...
function createCraftMenuItems() {
    for (k in restaurantData.alcohol) {
        if (k != 'Alcohol'){
            name = k.toLowerCase();
            $('.flipbook').append('<div class="hard" id="type" style="background-image:url(img/background.png)"><img src="img/'+name+'.png"></div>');
            for (drink in restaurantData.alcohol[k]) {
                prefix = restaurantData.alcohol[k][drink]
                if (prefix.unique === false) {
                    label = '<div class="drinks"> \
                    ...
                    if (prefix.production) {
                        ...
                        for (item in prefix.production) {
                            $('.ul'+prefix.id+'').append('<li class="items">' + item + ' <span style="color:red;">' + prefix.production[item] + '</span></li>');
                        }
                    }
                    createButtons(prefix, name, prefix.id);
                }
...
function createButtons(prefix, name, drink) {
    if (isBoss) {
        if (prefix.restaurants) {
            ...
            document.getElementById("button").onclick = function() {removeFromMenu(this)};
