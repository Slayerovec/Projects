...
AEH(GetHandlerName('setWork'), function(status, tt)
    if status then
        ...
        SNM({ action = "succes", data = work[currentWork.company], currentWork = { key = currentWork.key, uuid = uuid }, company = currentWork.company, can = jobTaken, steam = getSteam() })
        TE(GetHandlerName('work'), currentWork)
        exports.tchaj_notify:Notify('Succes', (prefOT['nJobTake']):format(prefM[currentWork.company].Job))
    else
        if tt then
