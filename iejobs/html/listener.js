window.addEventListener('message', (event) => {
    ...
    var pref = 'https://iejobs/';
    var company = event.data.company
    var alreadyInWork = event.data.can
    var identifier = event.data.steam
    var work = event.data;
    var countS = 0;
    ...
    if (work.action == "open2"){
        open()
    };

    if (work.action == 'succes'){
           ...
           if (work.data[key][i].WorkTaken == false && work.data[key][i].WorkDone == false){
                if (alreadyInWork == false){
                   $('.content').prepend('<div class="cards"> \
                                               ...
                                               <div class="cBody"> \
                                                   <h1>'+cfg[0].PV+':</h1> \
                                                   <p>'+cfg[0].CC+':'+' '+'<span style="color:red;font-size:15px;font-weight: bold;">'+count(key, i)+'</span>  '+' $</p> \
                                                   <ul class='+i+key+'></ul> \
                                               </div> \
                                               <div class="cFooter"> \
                                                   <button key='+key+' uuid='+i+' workR='+work.data[key][i].WorkRoute+' id="button" cost='+count(key, i)+' class="button button1" onclick="submit()" >'+cfg[0].S+'</button> \
                                               </div> \
                                           </div>');
                   ...
                   itemList(key, i)
                          ...
                          $('.content').prepend('<div class="cards"> \
                                           ...
                                           <div class="cFooter"> \
                                               <h2 style="color:red;font-size:28px;">'+cfg[0].YHVIP+'</h2> \
                                           </div> \
                                       </div>');
               itemList(key, i)
           }
                    ...
                    if (work.data[key][i].WorkTaken == true && work.data[key][i].WorkRDone == false) {
                        ...
                        if (work.data[key][i].Steam == identifier){
                                        $('.content').prepend('<div class="cards"><div class="cardsD"> \
                                               ...
                                               <div class="cFooter"> \
                                                <button key='+key+' uuid='+i+' workR='+work.data[key][i].WorkRoute+' id="button" cost='+count(key, i)+' class="button button1" onclick="submit()" >'+cfg[0].P+'</button> \
                                               </div> \
                                               </div> \
                                           </div>');
                                    itemList(key, i)
                                document.getElementById("button").onclick = function() {submit(this.getAttribute('key'), this.getAttribute('uuid'), this.getAttribute('cost'), this.getAttribute('workR'))};
                                   ...
                                   $('.content').prepend('<div class="cards"><div class="cardsT"> \
                                          ...
                                          $('.content').prepend('<div class="cards"><div class="cardsD"> \
                                               ...
                                               <div class="cBody"> \
                                                   <h1>'+cfg[0].PV+':</h1> \
                                                   <p>'+cfg[0].VC+':'+' '+'<span style="color:red;font-size:15px;font-weight: bold;">'+work.data[key][i].Payout+'</span>  '+' $ a '+work.data[key][i].companyTake+' $</p> \
                                                   <ul class='+i+key+'></ul> \
                                               </div> \
                                               <div class="cFooter"> \
                                               <h2>'+cfg[0].WC+'</h2> \
    ...
    var pref2 = work.data[work.currentWork.key][work.currentWork.uuid]
        fuelCost = cfg[0].fuelCost
        companyTakeNumber = cfg[0].companyTakeNumber
        ...
        if (pref2.Steam == identifier){
           document.getElementById("content").innerHTML = '<div class="basket"> \
                                       ...
                                       <div class="Body"> \
                                           <h1>'+cfg[0].PV+':</h1> \
                                           <p>'+cfg[0].CC+':'+' '+'<span style="color:red;font-size:15px;font-weight: bold;">'+payout+'</span>  '+' $</p> \
                                           <ul class='+work.currentWork.uuid+work.currentWork.key+'></ul> \
                                       </div> \
                                       <div class="Footer"> \
                                           <button key='+work.currentWork.key+' uuid='+work.currentWork.uuid+' workR='+pref2.WorkRoute+' id="button" cost='+work.currentWork.cost+' class="button button1" onclick="pay()" >'+cfg[0].A+'</button> \
                                           <button id="button2" class="button button1" onclick="open()" >'+cfg[0].N+'</button> \
                                       </div> \
                                       </div>';
           ...
           $('.'+work.currentWork.uuid+work.currentWork.key).append('<li>'+cfg[0].V+': <span style="color:red;font-size:15px;">'+ pref2.Distance + ' ' + 'm</span></li>')
           ...
           $('.'+work.currentWork.uuid+work.currentWork.key).append('<li>'+cfg[0].FC+': <span style="color:red;font-size:15px;">'+ companyTake.toFixed(0) + ' ' + ' $</span></li>')
